...
A green check has two possible meanings — "I looked and it was fine" and "I never looked" — and by default they are indistinguishable from the outside. Every gate must make the second meaning impossible to reach. The recurring instances: a test runner that exits 0 having discovered zero test files; a marker/grep assertion satisfied by the very artifact it was meant to reject; a report-consuming script that passes when the report directory is missing or empty; an assertion fed a sentinel instead of a measurement (`values.at(-1) ?? Infinity`); and a budget or threshold never calibrated against one real run, so it can only fire on a value nobody has ever observed. **Prove non-vacuity by mutation, not by inspection.** Reasoning about whether a gate *would* catch something is precisely how vacuous gates survive review; the only evidence that settles it is breaking the thing the gate protects on purpose, watching it go red, and reverting. Where a gate carries several distinct guarantees, mutate each one separately, and include at least one mutation that must leave it **green** — a gate that fails on every change is as uninformative as one that passes on all of them. State the mutations and their outcomes; "looks correct" is not a result. The audit questions: does the gate assert that it found something to measure **before** it asserts on the value? If you deleted the gate's input entirely, would it fail — or go green? Is the threshold traceable to a measured baseline, or invented? Retries make this worse rather than better: an intermittently red gate under `retries: 1` trains everyone to re-run instead of investigate, and the passing attempt reports a budget as met on evidence the first attempt never produced. Severity follows what the gate was protecting — a format/lint gate → **MEDIUM**; a test, migration, security, or performance gate → **HIGH**, because being the thing that noticed was the gate's entire purpose.

### SELF-ANCHORED-CHECK — verification criteria must come from the source, never from the result

`VACUOUS-PASS` covers a gate that never looked. This is its quieter sibling: a check that looked hard, in a mirror. When the criteria for verifying an artifact are extracted from that same artifact, the check is a tautology and cannot return anything but green — every anchor drawn from a rewrite is present in the rewrite by construction. It appears wherever output is checked against a list the checker also produced: a coverage check whose anchors come from the new file rather than the old one, a test whose expected values were read off the implementation it is testing, a migration verified against the types it generated, a summary audited against itself, a per-target claim resting on one target's observation. The distinguishing symptom is a suspiciously perfect score — 35/35, all green, nothing to report — on work that was never independently sampled. **Anchor the criteria to the source, the spec, the original, or the caller — anything the artifact under test did not produce.** For a restructured file that means a token-level diff of original against result (every code span, every content word), not a spot-check against remembered rules; the spot-check returned 35/35 while seven rules were missing, and the token diff found them at once. The audit questions: where did this check's list of things-to-verify come from, and could the artifact have influenced it? If the artifact were silently replaced with an empty one, would the check go red? Severity follows what went unverified, with a floor of **MEDIUM** — a self-anchored check is worse than no check at the same severity, because no check leaves the reader calibrated while a green one terminates the search.
...
**Every rule states a trigger, and names a path if it produces an artifact.** A rule phrased as a topic (`Recording`, "Inventory: `PROJECTS.md`") does not fire; a rule phrased as a condition ("before judging risk in an unfamiliar repo", "when two fixes have failed") does. And a rule that tells the reader to record something without naming the file never runs — that insight is `CLAUDE.md`'s own, and the 2026-08-19 rewrite violated it by mandating a fix log for every project while naming paths only for template-derived ones. Apply the test on every edit to this set and to `CLAUDE.md`: read each rule and name (a) the condition that makes it fire and (b) the file it writes to, if any. Three separate losses in that rewrite — an unnamed default fix-log path, a `PROJECTS.md` pointer decayed to a bare path, and a dropped backup-drill timing — were all this one defect, all three caught by hand when the test would have caught them mechanically.
